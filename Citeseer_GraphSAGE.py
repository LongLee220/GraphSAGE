...
class DataCenter(object):
    ...
    def load_Dataset(self,dataset='citeseer'):
        """读取存放在指定路径的数据集"""
        ...
        self.G = nx.Graph()
        if dataset == 'citeseer':
            content = self.file_paths['citeseer_content'] # 获取citeseer_content的地址
            cite = self.file_paths['citeseer_cites'] # 获取citeseer_cites的地址
...
## Main
file_paths = {'citeseer_content':'/home/gxzy/GNN/Graphsage/data/citeseer/citeseer.content','citeseer_cites':'/home/gxzy/GNN/Graphsage/data/citeseer/citeseer.cites'}
datacenter  = DataCenter(file_paths)
datacenter.load_Dataset()
feature_data = torch.FloatTensor(getattr(datacenter, 'citeseer'+'_feats'))
label_data = torch.from_numpy(getattr(datacenter,'citeseer'+'_labels')).long()
adj_lists = getattr(datacenter,'citeseer'+'_adj_lists')
random.seed(824)
np.random.seed(824)
