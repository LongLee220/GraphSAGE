...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 26 09:26:55 2022
...
class DataCenter(object):
    ...
    def load_Dataset(self,dataset='pubmed'):
        """读取存放在指定路径的数据集"""
        ...
        #label_list = np.empty((num_nodes, 1), dtype=np.int64)
        self.G = nx.Graph()
        if dataset == 'pubmed':
            content = self.file_paths['pubmed_content'] # 获取pubmed_content的地址
            cite = self.file_paths['pubmed_cite'] # 获取pubmed_cite的地址
            with open(content) as f1:
                
                f1.readline()
                feat_map = {entry.split(":")[1]:i-1 for i,entry in enumerate(f1.readline().split("\t"))}
                for i, line in enumerate(f1):
                    info = line.split("\t")
                    node_map[info[0]] = i
                    #label_list[i] = int(info[1].split("=")[1])-1
                    label = int(info[1].split("=")[1])-1
                ...
                for i,each_sample in enumerate(f1.readlines()): # 遍历每个样本的特征
                    ...
                    for word_info in sample_clean[2:-1]:
                        word_info = word_info.split("=")
                        feat_list.append(word_info[2:-1])
                    # 把节点名称映射为节点编号 
                    node_map[sample_clean[0]]=i
                    print(sample_clean[1].split("="))
                    label = int(sample_clean[1].split("=")[1])-1
            ...
            with open(cite) as f2:
                ...
                for j,each_pair in enumerate(f2.readlines()):
                    ...
                    self.G.add_edge(node_map[pair[0]],node_map[pair[1]])
                '''
            #有共同标签的增加一条边    
            Nodes = list(self.G.nodes())
            for ide1 in range(len(Nodes)):
                node1 = Nodes[ide1]
                label1= label_list[node1]
                for ide2 in range(ide1, len(Nodes)):
                    node2 = Nodes[ide2]
                    label2= label_list[node2]
                    com_nei = set(self.G[node1]).intersection(set(self.G[node2]))

                    if len(com_nei) !=0 and label1==label2:
                            adj_lists[node1].add(node2)
                            adj_lists[node2].add(node1)              

            print(len(adj_lists[1]))
...
class UnsupervisedLoss(object):
    ...
    def __init__(self, adj_lists, train_nodes, device):
        ...
        self.Degree = {}
        for ide in self.adj_lists.keys():
            self.Degree[ide]=len(self.adj_lists[ide])
            
        ##K-core
        k=0
        self.kcore = {}
        self.List_all ={}
        Node_list = list(self.adj_lists.keys())
                ...
                if len(Remove_list) != 0:
                    if k > 0:
                        for v in Remove_list:
                            ...
                            for u in list(self.adj_lists[v]):
                                self.Degree[u] = self.Degree[u] -1
                    Flag = True
                else:
                    Flag = False
            self.List_all[k] = List_k
            k = k + 1
    ...
    def _run_random_walks(self, nodes):
        for node in nodes: # 遍历每个节点
            ...
            for i in range(self.N_WALKS): # 每个节点会有N_WALKS次的随机游走
                curr_node = node # 
                k_value = self.kcore[curr_node]
                for j in range(self.WALK_LEN): # 每次随机游走走WALK_LEN的长度
                    neighs = list(set(self.adj_lists[int(curr_node)]).intersection(set(self.List_all[k_value])))
                    if len(neighs) != 0:
...
class GraphSage(nn.Module):
    ...
    def forward(self, nodes_batch):
        ...
        for i in range(self.num_layers):
            ...
            nodes_batch_layers.insert(0, (lower_layer_nodes, lower_samp_neighs, lower_layer_nodes_dict))  # type: ignore
    ...
    def _get_unique_neighs_list(self, nodes, num_sample=10):
        ...
        ##1.1从邻居和Kcore相同的里面提取候选点
        to_neighs = []
        for node in nodes:
            k_value  = self.kcore[node]
            can_node = set(self.adj_lists[int(node)]).intersection(set(self.List_all[k_value]))
            if len(can_node) != 0:
                to_neighs.append(can_node)
            else:
                to_neighs.append(self.adj_lists[int(node)])
...
def evaluate(dataCenter, ds, graphSage, classification, device, max_vali_f1, name, cur_epoch):
    ...
    if vali_f1 > max_vali_f1:
        ...
        test_acc = accuracy_score(labels_test, predicts.cpu().data)
...
## Main
file_paths = {'pubmed_content':'/home/gxzy/GNN/Graphsage/data/Pubmed-Diabetes/data/Pubmed-Diabetes.NODE.paper.tab','pubmed_cite':'/home/gxzy/GNN/Graphsage/data/Pubmed-Diabetes/data/Pubmed-Diabetes.DIRECTED.cites.tab'}
datacenter  = DataCenter(file_paths)
datacenter.load_Dataset()
feature_data = torch.FloatTensor(getattr(datacenter, 'pubmed'+'_feats'))
label_data = torch.from_numpy(getattr(datacenter,'pubmed'+'_labels')).long()
adj_lists = getattr(datacenter,'pubmed'+'_adj_lists')
random.seed(824)
np.random.seed(824)
...
graphSage = GraphSage(2, feature_data.size(1), 128, feature_data, getattr(datacenter, ds+'_adj_lists'), device, gcn='store_true', agg_func='MEAN')  # type: ignore
