...
class DataCenter(object):
    ...
    def load_Dataset(self,dataset='cora'):
        ...
        if dataset == 'cora':
            ...
            ###计算度值所占的比例
            for i in nx.degree_histogram(self.G):
                y_val.append(float(i)/int(len(Nodes)))



            for j in range(len(nx.degree_histogram(self.G))):
                x_val.append(j)

            ##画图
            plt.loglog(x_val, y_val,color = 'blue',linewidth = 2)
            plt.savefig('Degree_distribute.jpg')
            plt.close() 
...
class SageLayer(nn.Module):
	...
	def __init__(self, input_size, out_size, gcn=False): 
		...
		self.gcn = gcn
		#self.weight = Parameter(torch.FloatTensor(self.out_size, self.input_size if self.gcn else 2 * self.input_size)) #初始化权重参数w*input.T
...
## Main
file_paths = {'cora_content':'/home/gxzy/GNN/Graphsage/data/cora/cora.content','cora_cite':'/home/gxzy/GNN/Graphsage/data/cora/cora.cites'}
...
torch.cuda.manual_seed_all(3407)#824
learn_method = 'sup'
ds = 'cora'
epochs = 50
max_vali_f1=0
